#!/usr/bin/env python3
"""
Snowflake Multimodal Agents Lab - Simple Array Approach

This version uses a simple string-based approach to insert arrays into Snowflake.
"""
...
# Step 3: Load embeddings and store in Snowflake (Simple approach)
def load_embeddings_to_snowflake(conn):
    """Load pre-generated embeddings and store in Snowflake using simple string approach"""
    ...
    for doc in tqdm(embeddings_data):
        # Convert embedding to a simple array string format
        embedding_str = '[' + ','.join(map(str, doc['embedding'])) + ']'
        
        # Use a simple INSERT with string concatenation
        insert_query = f"""
        INSERT INTO multimodal_documents (key, width, height, embedding)
        VALUES ('{doc['key']}', {doc['width']}, {doc['height']}, {embedding_str})
        """
...
def get_information_for_question_answering(conn, user_query: str, serverless_url: str) -> List[str]:
    ...
    SELECT key, width, height,
           VECTOR_COSINE_SIMILARITY(embedding, {query_embedding_str}) as similarity_score
