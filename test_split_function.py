#!/usr/bin/env python3
"""
Test using SPLIT function instead of STRTOK_TO_ARRAY
"""
...
def test_vector_search_with_split(conn):
    """Test vector search functionality using SPLIT function"""
    print("\n=== Testing Vector Search with SPLIT Function ===")
...
def main():
    """Main test function"""
    print("🧪 Testing SPLIT Function Approach")
    print("=" * 60)
    ...
    try:
        ...
        # Test vector search with SPLIT
        print("\n2. Testing vector search with SPLIT...")
        search_success = test_vector_search_with_split(conn)
        
        print("\n" + "=" * 60)
        print("📊 TEST SUMMARY:")
        print(f"✅ Snowflake Connection: SUCCESS")
        print(f"✅ Embedding Loading: SUCCESS")
        print(f"{'✅' if search_success else '❌'} Vector Search with SPLIT: {'SUCCESS' if search_success else 'FAILED'}")
        
        if search_success:
            print("\n🎉 SPLIT FUNCTION SOLUTION WORKS!")
        else:
            print("\n❌ SPLIT function also failed.")
