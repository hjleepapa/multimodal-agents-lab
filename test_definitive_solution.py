#!/usr/bin/env python3
"""
Test the DEFINITIVE solution using stored procedures for vector search
"""
...
def test_vector_search_with_stored_procedure(conn):
    """Test vector search functionality using stored procedure"""
    print("\n=== Testing Vector Search with Stored Procedure ===")
    ...
    try:
        ...
        cursor.callproc("vector_search", [query_embedding_str, 3])
...
def main():
    """Main test function"""
    print("🧪 Testing DEFINITIVE Snowflake Solution (Stored Procedure Approach)")
    print("=" * 80)
    ...
    try:
        ...
        # Test vector search with stored procedure
        print("\n2. Testing vector search with stored procedure...")
        search_success = test_vector_search_with_stored_procedure(conn)
        
        print("\n" + "=" * 80)
        print("📊 TEST SUMMARY:")
        print(f"✅ Snowflake Connection: SUCCESS")
        print(f"✅ Embedding Loading: SUCCESS")
        print(f"{'✅' if search_success else '❌'} Vector Search with Stored Procedure: {'SUCCESS' if search_success else 'FAILED'}")
        
        if search_success:
            print("\n🎉 DEFINITIVE SOLUTION CONFIRMED!")
            print("The stored procedure approach works correctly.")
            print("This is the FINAL WORKING solution!")
        else:
            print("\n❌ Stored procedure approach failed.")
