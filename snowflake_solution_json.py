#!/usr/bin/env python3
"""
Snowflake Multimodal Agents Lab - JSON Storage Solution

This version uses JSON storage in VARIANT column for embeddings.
This avoids all the string parsing issues with scientific notation.
"""
...
# Step 3: Load embeddings and store in Snowflake (JSON approach)
def load_embeddings_to_snowflake(conn):
    ...
    # Insert documents with embeddings using JSON approach
    for doc in tqdm(embeddings_data):
        # Convert embedding to JSON string
        embedding_json = json.dumps(doc['embedding'])
        
        # Insert using JSON parsing in SQL
        insert_query = """
...
# Step 4: Vector Search Function (using JSON to VECTOR conversion in SELECT)
def get_information_for_question_answering(conn, user_query: str, serverless_url: str = None) -> List[str]:
    """
    Retrieve information using vector search to answer a user query.
    Converts JSON to VECTOR in the SELECT query.
    ...
    if serverless_url and serverless_url != "your-serverless-endpoint-url":
        ...
        if result:
            # The embedding is stored as VARIANT (JSON), so we can access it directly
