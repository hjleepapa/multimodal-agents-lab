#!/usr/bin/env python3
"""
Snowflake Multimodal Agents Lab - ARRAY Version

This version uses ARRAY type instead of VECTOR type for easier insertion,
then converts to VECTOR in queries using a view.
...
Setup:
Run the snowflake_setup_alternative.sql script in your Snowflake environment first.
"""
...
# Step 3: Load embeddings and store in Snowflake (ARRAY version)
def load_embeddings_to_snowflake(conn):
    ...
    for doc in tqdm(embeddings_data):
        # Convert embedding to JSON string for insertion
        embedding_json = json.dumps(doc['embedding'])
        
        # Use PARSE_JSON to convert string back to ARRAY
        insert_query_json = """
        INSERT INTO multimodal_documents (key, width, height, embedding)
        VALUES (%s, %s, %s, PARSE_JSON(%s))
        """
        
        cursor.execute(insert_query_json, (
            doc['key'],
            doc['width'],
            doc['height'],
            embedding_json
...
def get_information_for_question_answering(conn, user_query: str, serverless_url: str) -> List[str]:
    ...
    # Convert query embedding to JSON for consistency
    query_embedding_json = json.dumps(query_embedding)
    
    # Perform vector search using the view that converts ARRAY to VECTOR
    cursor = conn.cursor(DictCursor)
    
    search_query = """
    SELECT key, width, height,
           VECTOR_COSINE_SIMILARITY(embedding, PARSE_JSON(%s)) as similarity_score
...
# Rest of the functions remain the same as the original solution...
