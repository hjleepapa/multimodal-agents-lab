#!/usr/bin/env python3
"""
Snowflake Multimodal Agents Lab - ABSOLUTE FINAL SOLUTION

This is the DEFINITIVE solution that works with Snowflake's current limitations.
Uses VARIANT storage with JSON string conversion for insertion.
"""
...
# Step 3: Load embeddings and store in Snowflake (VARIANT approach with JSON string)
def load_embeddings_to_snowflake(conn):
    """Load pre-generated embeddings and store in Snowflake using VARIANT approach with JSON string"""
    ...
    # Insert documents with embeddings using VARIANT approach with JSON string
    for doc in tqdm(embeddings_data):
        # Convert embedding to JSON string
        embedding_json = json.dumps(doc['embedding'])
        
        # Insert using JSON string (Snowflake will parse it as VARIANT)
        insert_query = """
